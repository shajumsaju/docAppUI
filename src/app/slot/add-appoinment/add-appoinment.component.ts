...
//For Modal
  openModal(template: TemplateRef<any>, zone) {
    ...
    this.modalRef = this.modalService.show(
      template,
      Object.assign({}, { class: 'gray modal-lg product-modal' })
    );
  }
      ...
      userList.filter((userBasedVaue) => {
        slotLists.filter((daysBaseList) => {
          ...
          if (moment(slotDate).isSame(appoimentDate)) {
            if (daysBaseList.slotstartTime.indexOf(userBasedVaue.slotstartTime) > -1 &&
              daysBaseList.zone.toLowerCase().indexOf(userBasedVaue.zone) > -1) {
  ...
  fromTime(event) {
    ...
    if (hoursMini[0] != "00") {
      ...
      this.addslotForm.patchValue({ endTime: this.endslotTime })
  ...
  minutesTohours(value)
    {
      ...
      convertMinutes = minutes.toString().padStart(2, '0') + ':' +
  ...
  slotAdding() {
    ...
    this.commanService.post('/addSlot', params).subscribe((response) => {
      if (response)
        this.dayWiseList(this.selectedDate)
      this.modalRef.hide();
      this.createForm();
    })
